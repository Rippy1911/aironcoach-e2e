 ...
 *
 * Usage:
 *   node scripts/upload-wave2-release.mjs [--tag=wave2-YYYYMMDD]
 */
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import { execSync } from 'node:child_process';

const ARTIFACTS_DIR = path.resolve('test-results/wave2-artifacts');
const tag =
  process.argv.find((a) => a.startsWith('--tag='))?.split('=')[1] ??
  `wave2-${new Date().toISOString().slice(0, 10)}`;
...
async function main() {
  ...
  try {
    ...
    console.error(`No artifacts in ${ARTIFACTS_DIR}. Run wave2 e2e specs first.`);
    process.exit(1);
  }
  ...
  try {
    execSync(
      `gh release create "${tag}" ${files.map((f) => `"${f}"`).join(' ')} --repo Rippy1911/aironcoach-e2e --title "Wave 2 E2E artifacts ${tag}" --notes "Auto-uploaded Wave 2 nutrition deep-flow screenshots (FCU fallback)" --draft`,
      { stdio: 'inherit' },
    );
    const url = execSync(
