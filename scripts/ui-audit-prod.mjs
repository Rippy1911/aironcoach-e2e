...
const report = {
  ...
  checks: {
    ...
    archiveOfferLabel: /Archive offer|" Archive"|Archived — not visible/i.test(js),
    archivedVisibilityInUI: /visibility:\s*"archived"|Archived — not visible/.test(js),
    getCoachStatsRef: js.includes('getCoachStats'),
    hubLoadErrorCopy: js.includes("Couldn't load coach dashboard"),
    notCoachEmpty: js.includes("You're not a coach yet"),
    communityFeedMaxW: js.includes('max-w-[680px]'),
    widePageShell: js.includes('max-w-[1920px]') || js.includes('max-w-6xl'),
    ...
    presenceStatusSystem: js.includes('airon_presence_status'),
    presenceOptionsBusyDnd: js.includes('"online","busy","dnd","invisible"'),
    coachLifecycleDeactivate: js.includes('deactivateCoachProfile'),
    coachLifecycleReactivate: js.includes('reactivateCoachProfile'),
    coachLifecyclePauseLabel: js.includes('Pause coach profile'),
  },
  pages: {
